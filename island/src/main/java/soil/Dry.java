...
public class Dry implements SoilProfile {
    Structs.Mesh mesh;
    public Dry (Structs.Mesh mesh) {
        this.mesh = mesh;
    }
    //called by soilChooser
    public Dry(Map<String, String> options, Structs.Mesh aMesh){
        this(aMesh);
    }
            ...
            if((typePoly.get().equals("land"))) {
                ...
                for (Integer j : p.getNeighborIdxsList()) {
                    ...
                    if (type.isPresent() && ((type.get().equals("aquifer") || type.get().equals("lake")))) {
                        ...
                        String absorption = String.valueOf(new DryCalculator().calculate(moistness.get()));
                        ...
                        Structs.Polygon dryPoly = builder.addProperties(abs).addProperties(newType).build();
                        updatedPolygons.set(i, dryPoly);
                    }
                }
            }
            i++;
