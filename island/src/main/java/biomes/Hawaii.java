...
public class Hawaii implements Biome {

    Structs.Mesh mesh;

    public Hawaii(Structs.Mesh mesh) {
        this.mesh = mesh;
    }

    //called by soilChooser
    public Hawaii(Map<String, String> options, Structs.Mesh aMesh) {
        this(aMesh);
    }
    @Override
    public List<Structs.Polygon> makeBiome(List<Structs.Polygon> absorbedPolygons){
        ...
        for(Structs.Polygon p: absorbedPolygons){
            ...
            if(typePoly.isPresent() && (typePoly.get().equals("absorbed"))){
                ...
                String color = "235,196,120"; //add hawaii color
